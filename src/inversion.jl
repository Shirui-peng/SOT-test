...
*T*-wave delays `Δτ`.
"""
function correctcycleskipping(eqname, tstations, tpairs, ppairs, E, Rxx, Rnn, P, m)
...
#  R = I - E*A
  ...
  return collect(eachrow(reshape(y[1:l*nt], (nt, l)))), Δτp#, collect(eachrow(reshape(Δr, (nt, l))))
...
#  R = I - E*A
  ...
  # return optimal delays
  return collect(eachrow(reshape(y[1:l*nt], (nt, l)))), Δτp, cs, x1,x2,Δτ2#, collect(eachrow(reshape(Δr, (nt, l))))
...
function regulargrid(td, θd, ti, θi, a, R, iRyy, E, λt, λθ, στ)
  ...
  ω = 2π/SOT.meanyear
  p = R\a
  #corr(t1, θ1, t2, θ2) = abs(t1-t2)>tavg/2 ? 2λt/tavg*exp(-abs(t1-t2)/λt)*sinh(tavg/2λt) : λt/tavg*(2-exp((t1-t2-tavg/2)/λt)-exp(-(t1-t2+tavg/2)/λt))
...
""" interpolation onto regular grid of averages """
function regulargridold(td, ti, a, R, invR, P, σc, ctau, lags)
  @printf("\n")
  @printf("interpolation onto regular grid\n")
  m = length(td)
  mi = length(ti)
  ...
  #C = Matrix([C[1] C[2] C[4]; C[3] C[6] C[7]; C[5] C[8] C[9]])

  F = [C zeros(l*length(ti), m) zeros(l*length(ti), l)]
  F = [F*invR; zeros(l*length(ti),(l+1)*m) kron(I(l), tid.-tm)]
  Di = [I(l*length(ti)) I(l*length(ti))]
  τi = reshape(Di*F*a, (length(ti), l))# reshape(F*p, (length(ti), l)) + a[(l+1)*m+1:(l+1)*m+l]'.*(tid.-tm) 

  Pi = Di*F*P*(Di*F)'#Ci - Riy*invRyy*Riy'
  # difference matrix 
  #Di = [zeros(l*mi,(1+l)*m) kron(I(l), tid.-tm) I(l*mi)]
